...
### 0.2 สร้างโครงสร้างโฟลเดอร์และไฟล์ __init__.py

**สร้างโฟลเดอร์ทั้งหมดพร้อม __init__.py:**

```bash
mkdir -p data/raw data/processed models notebooks configs results tests \
         src/data src/features src/models src/utils && \
touch src/__init__.py src/data/__init__.py src/features/__init__.py \
      src/models/__init__.py src/utils/__init__.py tests/__init__.py
```

> 💡 **อธิบาย:** 
> - `mkdir -p` สร้างโฟลเดอร์หลายระดับพร้อมกัน
> - `\` ใช้ต่อคำสั่งข้ามบรรทัด
> - `&&` รันคำสั่งถัดไปเมื่อคำสั่งก่อนหน้าสำเร็จ
...
### 0.6 ดูโครงสร้างโปรเจกต์ที่สมบูรณ์
