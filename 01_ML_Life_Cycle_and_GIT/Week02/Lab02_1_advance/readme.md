...
> 📝 **หมายเหตุ:** ใช้ `tree` (ไม่มี `-a`) เพื่อซ่อนโฟลเดอร์ที่ขึ้นต้นด้วยจุด
...
**🌳 ตรวจสอบโครงสร้างหลังสร้าง .gitkeep:**
```bash
...
> 📝 **อธิบายคำสั่ง tree:**
> - `-a` : แสดงไฟล์ที่ซ่อน (ขึ้นต้นด้วย `.`)
> - `--dirsfirst` : แสดงโฟลเดอร์ก่อนไฟล์
> - `-I '.git'` : ซ่อนโฟลเดอร์ `.git`
...
**🌳 ตรวจสอบโครงสร้างหลังสร้างไฟล์พื้นฐานทั้งหมด:**
```bash
tree -a --dirsfirst -I '.git'
```
...
**🌳 ตรวจสอบโครงสร้างโฟลเดอร์ config:**
```bash
tree config/
```
...
**🌳 ตรวจสอบโครงสร้างโฟลเดอร์ src หลังเพิ่ม train.py:**
```bash
tree src/
```
...
**🌳 ตรวจสอบโครงสร้างหลัง training (รวมไฟล์ที่ถูก generate):**
```bash
tree -a --dirsfirst -I '.git'
```
...
├── models
│   ├── .gitkeep
│   └── model.joblib          ← ไฟล์ model ที่ถูกสร้าง
├── results
│   └── .gitkeep
├── src
│   ├── __pycache__           ← Python cache
│   │   └── data_loader.cpython-xx.pyc
│   ├── data_loader.py
│   └── train.py
├── .gitignore
├── README.md
...
> 📝 **สังเกต:** ไฟล์ `model.joblib` และ `__pycache__/` จะถูก ignore ด้วย .gitignore
...
**🌳 ตรวจสอบโครงสร้างโฟลเดอร์ src หลังเพิ่ม evaluate.py:**
```bash
...
**🌳 ตรวจสอบโครงสร้างทั้งหมดหลัง evaluation:**
```bash
tree -a --dirsfirst -I '.git|__pycache__'
```
...
├── models
│   ├── .gitkeep
│   └── model.joblib          ← ถูก ignore
├── results
│   ├── .gitkeep
│   ├── confusion_matrix.png  ← ถูก ignore (ผลลัพธ์)
│   └── metrics.json          ← ถูก ignore (ผลลัพธ์)
├── src
│   ├── data_loader.py
│   ├── evaluate.py
│   └── train.py
...
> 📝 **สังเกต:** ไฟล์ใน `results/` และ `models/` (ยกเว้น `.gitkeep`) จะถูก ignore
...
**🌳 ดูโครงสร้างไฟล์ที่ถูก track โดย Git:**
```bash
...
### 🌳 คำสั่ง tree ที่ใช้ในแล็บ

| คำสั่ง | คำอธิบาย |
|--------|----------|
| `tree` | แสดงโครงสร้างโฟลเดอร์พื้นฐาน |
| `tree -a` | แสดงรวมไฟล์ที่ซ่อน (เริ่มด้วย `.`) |
| `tree -a -I '.git'` | ซ่อนโฟลเดอร์ `.git` |
| `tree --dirsfirst` | แสดงโฟลเดอร์ก่อนไฟล์ |
| `tree -L 2` | แสดงลึกแค่ 2 ระดับ |
| `tree src/` | แสดงเฉพาะโฟลเดอร์ `src` |
| `tree -I '__pycache__\|*.pyc'` | ซ่อนหลายรูปแบบ |

**ตัวอย่างการใช้รวมกัน:**
```bash
# แสดงทั้งหมด ยกเว้น .git และ __pycache__ โดยเรียงโฟลเดอร์ก่อน
tree -a --dirsfirst -I '.git|__pycache__'
```
