...
* ให้ $P_{\text{train}}(X)$ คือการแจกแจงของข้อมูลนำเข้า $X$ ตอนฝึกโมเดล  
* ให้ $P_{\text{prod}}(X)$ คือการแจกแจงของข้อมูลนำเข้า $X$ ตอนใช้งานจริง (production)
...
Model Drift เป็นคำที่ใช้เรียกการเสื่อมสภาพของประสิทธิภาพโมเดลโดยรวม ซึ่งมักเกิดจาก Data Drift หรือ Concept Drift (หรือทั้งสองอย่าง). มันไม่ใช่ประเภท drift ที่แยกต่างหาก แต่เป็นผลกระทบที่สังเกตได้จากความเปลี่ยนแปลงในข้อมูลหรือความสัมพันธ์.

**ตัวอย่างง่าย:**

* โมเดลมี accuracy จากเดิม $95\%$ แต่หลังผ่านไป 6 เดือน เหลือเพียง $75\%$ เนื่องจากเกิด Data Drift หรือ Concept Drift ที่ทำให้ distribution ของข้อมูลหรือความสัมพันธ์เปลี่ยนแปลง → เรียกว่า model drift โดยรวม
...
| ----------------- | --------------------------------------------------- | ------------------------------------------------------------- | ------------------------ | -------------------------- |
| **Data Drift**    | เปลี่ยน $P_{\text{train}}(X) \neq P_{\text{prod}}(X)$ | คงเดิม                                                      | อาจเพิ่มขึ้น             | ผู้ใช้/เซ็นเซอร์เปลี่ยน    |
| **Concept Drift** | คงเดิม                                              | เปลี่ยน $P_{\text{train}}(Y \mid X) \neq P_{\text{prod}}(Y \mid X)$ | เพิ่มขึ้นชัดเจน          | ความสัมพันธ์โลกจริงเปลี่ยน |
| **Model Drift**   | อาจเปลี่ยนหรือไม่                                   | อาจเปลี่ยนหรือไม่                                           | $E_{\text{prod}} \uparrow$ | ผลกระทบโดยรวมจาก drift อื่น ๆ |
