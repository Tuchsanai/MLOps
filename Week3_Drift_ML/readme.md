...
* ให้ $P_{train}(X)$ คือการแจกแจงของข้อมูลนำเข้า $X$ ตอนฝึกโมเดล
* ให้ $P_{prod}(X)$ คือการแจกแจงของข้อมูลนำเข้า $X$ ตอนใช้งานจริง (production)
...
* ตอนฝึก โมเดลแนะนำสินค้าจะเจอ distribution ของราคา:
  ...
  $$
  P_{train}(price) \sim \mathcal{N}(500, 100^2)
  $$
  
* แต่เมื่อใช้งานจริง (เช่น ช่วงโควิด) ลูกค้าเริ่มซื้อของแพงขึ้น:

  $$
  P_{prod}(price) \sim \mathcal{N}(800, 200^2)
  $$

ผลคือโมเดลสับสนเพราะราคาเฉลี่ยเปลี่ยน แม้ “พฤติกรรมซื้อ = label” ยังสัมพันธ์เดิม
...
* ให้ **ค่าเฉลี่ยของ error ตอน train/test เดิม** คือ:
  ...
  $$
  E_{train} = \mathbb{E}_{(x,y) \sim P_{train}} [L(y, f(x))]
  $$
* ให้ **ค่าเฉลี่ยของ error ตอน production** คือ:

  $$
  E_{prod} = \mathbb{E}_{(x,y) \sim P_{prod}} [L(y, f(x))]
  $$
...
| ----------------- | --------------------------------------- | ----------------------------------------------------- | ------------------- | -------------------------- |
| **Data Drift**    | เปลี่ยน $P_{train}(X) \neq P_{prod}(X)$ | คงเดิม                                                | อาจเพิ่มขึ้น        | ผู้ใช้/เซ็นเซอร์เปลี่ยน    |
| **Concept Drift** | คงเดิม                                  | เปลี่ยน $P_{train}(Y \mid X) \neq P_{prod}(Y \mid X)$ | เพิ่มขึ้นชัดเจน     | ความสัมพันธ์โลกจริงเปลี่ยน |
| **Model Drift**   | อาจเปลี่ยนหรือไม่                       | อาจเปลี่ยนหรือไม่                                     | $E_{prod} \uparrow$ | ผลกระทบโดยรวมจาก drift อื่น ๆ |
