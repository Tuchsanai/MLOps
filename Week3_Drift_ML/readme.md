...
## 2. Model Drift (การเสื่อมประสิทธิภาพโมเดล)
...
แม้ว่า $P_{train}(X) \approx P_{prod}(X)$ ก็ตาม → แสดงว่าโมเดลเอง “เสื่อม” ไม่ generalize แล้ว

**ตัวอย่างง่าย:**

* โมเดลมี accuracy จากเดิม $95\%$ แต่หลังผ่านไป 6 เดือน เหลือเพียง $75\%$ ทั้งที่ distribution ของข้อมูลยังคล้ายเดิม → เรียกว่า model drift

---

## 3. Concept Drift (การเปลี่ยนแปลงความสัมพันธ์ Input–Output)
...
| **Model Drift**   | ไม่เปลี่ยนมาก                           | ไม่เปลี่ยนมาก                                         | $E_{prod} \uparrow$ | โมเดลเสื่อม generalization |
