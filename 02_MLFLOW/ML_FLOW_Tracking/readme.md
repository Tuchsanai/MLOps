...
║   │                 │   │                 │   │                 │   │               │
║   │  "บันทึกและ     │   │  "แพ็คเกจโค้ด   │   │  "บันทึกและ     │   │ "จัดการ       │
║   │   ติดตามผล      │   │   ให้ทำซ้ำได้"  │   │   Deploy Model" │   │  เวอร์ชัน"    │
║   │   การทดลอง"     │   │                 │   │                 │   │               │
║   └────────┬────────┘   └────────┬────────┘   └────────┬────────┘   └───────┬───────┘
║            │                     │                     │                    │
║            ▼                     ▼                     ▼                    ▼
║   ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐   ┌───────────────┐
║   │ • Parameters    │   │ • MLproject     │   │ • Model Format  │   │ • Versioning  │
║   │ • Metrics       │   │ • conda.yaml    │   │ • Model Flavors │   │ • Stage Mgmt  │
║   │ • Artifacts     │   │ • Reproducible  │   │ • Model Serving │   │ • Lineage     │
║   │ • Web UI        │   │ • Git Support   │   │ • Multi-Deploy  │   │ • Approval    │
║   └─────────────────┘   └─────────────────┘   └─────────────────┘   └───────────────┘
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝
    ...
    ┌──────────────────────────────────────────────────────────────────────────┐
    │                                                                          │
    │   🔬 Experiment    →    📦 Package    →    🚀 Deploy    →    📋 Manage   │
    │   (Tracking)            (Projects)         (Models)         (Registry)   │
    │                                                                          │
    └──────────────────────────────────────────────────────────────────────────┘
```
...
├─────────────┬─────────────┬─────────────┬─────────────┬─────────────┤
│   sklearn   │   pytorch   │ tensorflow  │   xgboost   │    onnx     │
├─────────────┼─────────────┼─────────────┼─────────────┼─────────────┤
│   keras     │   spark     │   lightgbm  │   catboost  │   custom    │
└─────────────┴─────────────┴─────────────┴─────────────┴─────────────┘
```
...
**ปัญหาที่พบบ่อยเมื่อไม่มี Model Registry:**

```
😰 สถานการณ์ที่ไม่มี Model Registry:

"model ที่รันอยู่บน Production ตอนนี้คือไฟล์ไหน?"
    → ไม่รู้... น่าจะเป็น model_final_v2_new.pkl?

"model นี้ train ด้วย hyperparameters อะไร?"
    → จำไม่ได้... ลองหาใน notebook ดูนะ

"model เมื่อวานดีกว่า วันนี้ rollback ได้ไหม?"
    → ไม่ได้... ลบไฟล์เก่าไปแล้ว 😭

"ใครเป็นคน deploy model นี้? เมื่อไหร่?"
    → ไม่รู้... ไม่มี log
```

```
😊 สถานการณ์ที่มี Model Registry:

"model ที่รันอยู่บน Production ตอนนี้คือไฟล์ไหน?"
    → FraudDetector version 3, stage: Production ✅

"model นี้ train ด้วย hyperparameters อะไร?"
    → คลิกดู lineage → มาจาก Run abc123, lr=0.001, epochs=100 ✅

"model เมื่อวานดีกว่า วันนี้ rollback ได้ไหม?"
    → ได้เลย! เปลี่ยน version 2 เป็น Production แทน ✅

"ใครเป็นคน deploy model นี้? เมื่อไหร่?"
    → john@company.com, 2024-01-15 14:30:00 ✅
```
