...
### ✅ กรณีที่ 1: มี `mlflowserver-lab` อยู่แล้ว

ถ้าคุณเคยทำ Lab `mlflowserver-lab` มาก่อนและมีโฟลเดอร์อยู่แล้ว:

```bash
# เข้าไปในโฟลเดอร์ mlflowserver-lab ที่มีอยู่
cd mlflowserver-lab

# เปิดใช้งาน Virtual Environment
source .venv/bin/activate

# เปิด MLflow Server (ถ้ายังไม่ได้รัน)
mlflow server \
...
> 💡 **Tip:** ถ้า Server รันอยู่แล้ว สามารถข้ามไปเริ่ม Lab ได้เลย!

---

### 🆕 กรณีที่ 2: ยังไม่มี MLflow Server

ถ้ายังไม่เคยทำ `mlflowserver-lab` ให้ทำตามขั้นตอนด้านล่าง:

<details>
<summary><b>📖 คลิกเพื่อดูขั้นตอนการติดตั้ง MLflow Server</b></summary>
...
# อัพเดท pip ให้เป็นเวอร์ชันล่าสุด
...
# เปิด Server พร้อม SQLite Backend
mlflow server \
  --host 127.0.0.1 --port 8080 \
  --backend-store-uri sqlite:///mlruns_db/mlflow.db \
  --artifacts-destination ./mlartifacts \
...
เมื่อเห็นหน้า MLflow UI ที่ `http://127.0.0.1:8080` แล้ว คุณพร้อมเริ่ม Lab!
